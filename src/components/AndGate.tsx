...
export default function AndGate({ inputA, inputB, setInputA, setInputB }: AndGateProps) {
      ...
      <div className="flex items-center justify-center gap-4 w-full h-48 relative">
        ...
        <div className="flex-1 max-w-60">
          <svg viewBox="0 0 200 100" className="w-full h-auto overflow-visible">
            <path d="M 10,25 L 70,25" fill="none" strokeWidth="4" className={`transition-colors duration-200 ${inputA ? activeColor : inactiveColor}`} />
            <path d="M 10,75 L 70,75" fill="none" strokeWidth="4" className={`transition-colors duration-200 ${inputB ? activeColor : inactiveColor}`} />
            <path d="M 140,50 L 190,50" fill="none" strokeWidth="4" className={`transition-colors duration-200 ${output ? activeColor : inactiveColor}`} />
            <path d="M 70,15 L 105,15 A 35,35 0 0,1 105,85 L 70,85 Z" fill="#111827" stroke="#4b5563" strokeWidth="3" />
            <text x="90" y="56" fill="#4b5563" className="font-sans font-bold text-lg pointer-events-none select-none">&</text>
          </svg>
        </div>
